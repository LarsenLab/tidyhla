...
## 2. Create the Package Structure: Use usethis to create the package structure.
...
## 4. Load data into an R package

```{r}
# use_data(hla_data, overwrite =T)
```

## 5. R Markdown template file in the package

```{r}
# use_rmarkdown_template(("HLA Examples"))
```

## 6. Store a function in a file using `dump`
...
# use_package(package = "tidyhla", type = "Imports", min_version = NULL)
# use_package(package = "dplyr", type = "Depends", min_version = NULL)

#roxygenize()
```

## 8. R package vignettes

```{r}
# usethis::use_vignette("tidyhla")
...
#use_test(name = "clean_hla_class1")

#Documenting a data tile
#use_r("hla_data")
```

## 10. Check stauts of the package
```{r}
#usethis::use_description()
#devtools:: check()
```
